...
audience: specialist
description: Use as the first pass on a new Solidity engagement, before any scanner runs, building an audit-readiness x-ray that enumerates permissionless entry points, maps every actor and privileged role, and extracts the protocol's stated invariants so downstream passes have a target list. Reach for it when asked to x-ray, prep, or find where to start on a protocol.
---
...
outcome: nSLOC, entry points, permissionless count, roles, invariants, integrations, branch@commit.
