...
| This skill (0–8) | What it actually is |
|---|---|
| Phase 0 | Scope lock, program truth, facts |
| Phase 1–2 | Prior-art exclusion, planning, measured index |
| **Phase 3 / 3a / 3b** | **Hypothesis generation and hunting — 3b is invention** |
| Phase 4 | Chaining / composition |
| Phase 5 | Proof, PoC, negative controls |
| Phase 6–7 | Impact bar, cross-family reproduction, skeptic |
| Phase 8 | Package, de-AI, operator submit gate |

The contract S-stage axis (`S0`–`S7`) is owned by `scripts/python/verification_contract.py` and is
**not** a parallel numbering of these phases — resolve a bare `S`-label against that file, not by lining
it up against this table.

The mode axis is owned by `shared/modes/bounty.md` (phases 0–7) and is **not restated here** — resolve
a bare mode "Phase N" against that file, lining it up through the *What it actually is* column.
...
- **All four observable predicates hold**, scored from concrete signals (oracle
