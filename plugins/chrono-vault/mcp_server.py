...
# Declared so the generated tool schema carries a JSON-Schema `enum` and a bad
# value is rejected before the handler runs. The closed sets these mirror
# (notes.NOTE_TYPES, notes.STATUSES, lifecycle.OUTCOMES) stay authoritative and
# are still enforced server-side; tests assert the two never drift apart.
NoteType = Literal["attempt", "finding", "learning"]
NoteStatus = Literal[
    "candidate", "verified", "superseded", "invalidated", "archived"
]
UsageOutcome = Literal["used", "not_useful", "incorrect"]
...
def record(
    ...
    *,
    ...
    `title` and `body` are always required. `target` and `attack_class`
    describe an attack: `attempt` and `finding` require them, `learning` does
    not. `fields` carries any remaining canonical frontmatter (status,
    sensitivity, source_task, component, keywords, evidence_refs) and may also
    carry the four named parameters above, so existing callers keep working.
    """
    ...
    for field, value in named:
        ...
        if field in merged and merged[field] != value:
            raise ValueError(
                f"{field} was supplied both directly and in fields with "
                "different values"
            )
        merged[field] = value
    return record_note(note_type, merged)
...
def _run_mcp_server(server: FastMCP) -> None:
    """Serve private memory over stdio only.

    CB-26: the removed environment-selected SSE branch bound an unauthenticated
    HTTP listener over the whole vault, while the clearance model assumes a
    single trusted stdio parent. Any future network transport must arrive with
    its own authentication and clearance re-derivation.
    """
    server.run()
