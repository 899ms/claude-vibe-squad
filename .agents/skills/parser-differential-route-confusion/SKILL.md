...
audience: specialist
description: "Use during an authorized API or gateway assessment when routing or authorization may parse a path differently from downstream execution: test encoded and nested path forms, prove safe-route validation reaches a privileged handler, and chain only if the real target yields controlled impact."
---
