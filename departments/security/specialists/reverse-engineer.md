...
- **Evidence-gate.** A vulnerability hypothesis is a lead until reproduced inside isolation under **all four observable predicates** — oracle match, control separation, clean-snapshot repeat stability, production-fidelity harness (the gate `impact-validator` defines; `systematic-attacking` Law 2) — and settled cross-family; then it hands to `exploit-developer` / `impact-validator`. Outputs are analytical evidence, never weaponized derivatives.
