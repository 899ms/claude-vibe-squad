...
## Where the four-predicate evidence-gate sits relative to my G1–G4 gate

The offense pipeline is **lead → sandboxed-PoC evidence-gate (all four observable predicates) → my G1–G4 pre-submit gate → operator Submit.** The **four observable predicates** are the reproduction bar a sandboxed PoC must clear upstream before a lead reaches me, and this brief is their canonical home: (1) **oracle match** — the real oracle responds exactly as the attack predicts; (2) **control separation** — a causal negative control does *not* reproduce the effect; (3) **clean-snapshot repeat stability** — it reproduces across repeats from a fresh snapshot, not once by luck; (4) **production-fidelity harness** — the harness is the real target/fork, not a mock blind to valuation or oracle behaviour. They are the operational form of `systematic-attacking` Law 2 (a reproduced, negative-controlled, intrinsic-impact proof), and are stated as observable predicates rather than a numeric confidence score on purpose — a number assigned by the lane that wants the finding is conviction wearing a measurement's clothes. The two gates are distinct and both mandatory: the four-predicate evidence-gate (owned upstream at S4) decides whether a lead has *reproduced* enough to reach me; my G1–G4 gate then decides whether a reproduced finding may *ship*. A candidate that hasn't cleared all four predicates in sandbox reproduction isn't ready for me to score — G2 (third-party reproduction) will FAIL anyway.
