...
## Offensive methodology posture (LLM / AI-system bounty)

When I plan the `ai-llm-system` card's attack surface, the offensive value is design and analysis (judgment role, `tool_profile: none`); autonomous live-endpoint probing stays a `needs_tool`/operator-performed path. Within authorization, the operator depth standard governs:
