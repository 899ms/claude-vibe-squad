...
from gen_runtime_tool_summary import (
    RuntimeToolSummaryError,
    canonical_server,
...
GEMINI_PROJECTION_BEGIN = "# BEGIN SPECIALIST CAPABILITY PROJECTION"
GEMINI_PROJECTION_END = "# END SPECIALIST CAPABILITY PROJECTION"
GEMINI_PROJECTION_KEYS = {
    "capability_source",
...
def _gemini_comment_projection(text: str, path: Path) -> dict[str, Any]:
    """Read one generated Gemini projection from its post-frontmatter comment."""
    begin_matches = list(
    ...
    if len(begin_matches) != 1 or len(end_matches) != 1:
        raise CapabilityHomeError(
            f"{path}: Gemini capability projection requires exactly one begin/end sentinel"
        )

    begin = begin_matches[0]
    end = end_matches[0]
    frontmatter_end = text.find("\n---", 4) if text.startswith("---\n") else -1
    if frontmatter_end < 0 or begin.start() <= frontmatter_end + 4:
        raise CapabilityHomeError(
            f"{path}: Gemini capability projection must follow closed Markdown frontmatter"
        )
    if end.start() <= begin.end():
        raise CapabilityHomeError(
            f"{path}: Gemini capability projection end sentinel precedes its begin sentinel"
        )

    comment_open = text.rfind("<!--", 0, begin.start())
    comment_close_before = text.rfind("-->", 0, begin.start())
    comment_close = text.find("-->", comment_open + 4)
    if (
        comment_open < 0
        or comment_open <= frontmatter_end + 4
        or comment_open < comment_close_before
        or comment_close < 0
        or comment_close < end.end()
        or text.find("<!--", comment_open + 4, comment_close) >= 0
    ):
        raise CapabilityHomeError(
            f"{path}: Gemini capability projection must be inside one HTML comment"
        )

    result: dict[str, Any] = {}
    comment_header_end = text.find("\n", comment_open, begin.start())
    if comment_header_end >= 0:
        comment_header = text[comment_open + 4 : comment_header_end]
        generated_by = re.search(
            r"(?:^|\s)generated_by=([A-Za-z0-9./_-]+)(?:\s|$)",
            comment_header,
        )
        if generated_by:
            result["generated_by"] = generated_by.group(1)
    ...
    for line in text[begin.end() : end.start()].splitlines():
        ...
        if line[0].isspace() or ":" not in line:
            raise CapabilityHomeError(
                f"{path}: malformed Gemini capability projection line {line!r}"
            )
        key, raw = line.split(":", 1)
        key = key.strip()
        raw = raw.strip()
        if key not in GEMINI_PROJECTION_KEYS:
            raise CapabilityHomeError(
                f"{path}: unexpected Gemini capability projection field {key}"
            )
        if key in result:
            raise CapabilityHomeError(
                f"{path}: duplicate Gemini capability projection field {key}"
            )
        if key.startswith("capability_") and key not in {
            "capability_source",
            "capability_source_sha256",
        }:
            try:
    ...
    required = {"capability_source", "capability_source_sha256"}
    missing = sorted(required - result.keys())
    if missing:
        raise CapabilityHomeError(
            f"{path}: Gemini capability projection missing {', '.join(missing)}"
        )
    return result
...
def canonical_brief(root: Path | None, row: dict[str, str]) -> Path:
    specialist = row["specialist"]
    namespace = row["source_namespace"]
    shared = Path("shared/specialists") / f"{specialist}.md"
    if namespace == "shared" or (root is not None and (root / shared).is_file()):
        return shared
    ...
    for lane, path in _adapter_globs(root):
        ...
        try:
            ...
            if lane == "gpt-codex":
                ...
                parsed = _markdown_frontmatter(text, path)
                frontmatter_projection = GEMINI_PROJECTION_KEYS.intersection(parsed)
                comment_projection = _gemini_comment_projection(text, path)
                if frontmatter_projection and comment_projection:
                    raise CapabilityHomeError(
                        f"{path}: Gemini capability projection is ambiguous across "
                        "frontmatter and comment"
                    )
                if not frontmatter_projection:
                    parsed.update(comment_projection)
            else:
...
def baseline_text(root: Path, baseline_ref: str, relative: Path) -> str:
    ...
    if result.returncode != 0:
        ...
        if current.is_file() and missing_at_ref:
            return ""
        raise CapabilityHomeError(
            ...
            for identifier in sorted(
                ...
                if not retained:
                    issues.append(
                        diagnostic(
                            "migration-parity",
                            canonical_brief(None, row).as_posix(),
                            identifier,
    ...
    for (specialist, lane), entry in sorted(source_entries.items()):
        ...
        for kind in CAPABILITY_FIELDS:
            for ref in entry[kind]:
                ...
                # above. (superseded is deliberately excluded: a dead/duplicated
                # tool may legitimately remain installed while a live capability
...
def required_primary_diagnostics(
    ...
    *,
    ...
    adapters: dict[tuple[str, str], dict[str, Any]] | None = None,
) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    ...
    try:
        projection = project_runtime_tools(repo_root)
        expected_runtime = render_runtime_map(repo_root)
    except RuntimeToolSummaryError as exc:
        return [
            diagnostic(
                "runtime-tool-summary",
                RUNTIME_MAP_RELATIVE.as_posix(),
                "projection",
                str(exc),
                kind="schema",
            )
        ]
    actual_runtime = (repo_root / RUNTIME_MAP_RELATIVE).read_text(encoding="utf-8")
    if actual_runtime != expected_runtime:
        issues.append(
            diagnostic(
                ...
                RUNTIME_MAP_RELATIVE.as_posix(),
                "generated-columns",
                "required_tools/preferred_tools differ from the capsource projection",
                kind="schema",
            )
        )
    ...
    for specialist, row in sorted(rows.items()):
        ...
        if entry is None:
            continue
        adapter = source_adapters.get((specialist, primary))
        for level in ("required_tools", "preferred_tools"):
            for identifier in projection[specialist][level]:
                assignments = [
                    ref
                    for ref in entry["mcps"]
                    if canonical_server(ref.identifier) == identifier
                    and is_usable_capability("mcps", ref.availability)
                ]
                if not assignments:
                    issues.append(
                        diagnostic(
                            ...
                            "generated server requirement lacks a usable primary-lane assignment",
                ...
                if adapter is None or assigned_ids.isdisjoint(adapter["mcps"]):
                    issues.append(
                        diagnostic(
                            ...
                            SOURCE_RELATIVE.as_posix(),
                            f"{specialist}:{primary}:{identifier}",
                            "assigned provider is absent from the generated adapter",
                            kind=level.removesuffix("_tools"),
                        )
                    )
    return issues
    ...
    if source_entries is not None:
        ...
        for (specialist, lane), entry in sorted(source_entries.items()):
            ...
            assigned_servers = {
                ...
                for ref in entry["mcps"]
                if is_usable_capability("mcps", ref.availability)
            }
            primary_plan = []
            if lane == primary_lane:
                for level in ("required_tools", "preferred_tools"):
                    for identifier in runtime_summary[specialist][level]:
                        primary_plan.append(
                            {
                                "capability_id": assigned_servers[identifier],
                                "evidence": "capsource-generated-runtime-tool-summary",
                                "id": identifier,
                                "kind": "mcps",
                                "provider_lane": lane,
                                "requirement": level.removesuffix("_tools"),
                                "resolution": "local",
                            }
                        )
            source_index.append(
