...
A primitive is not a critical until the **terminus** (funds moved) is reachable at no privilege. For nonce/reorg bugs: only the bridge signs for its address, so a distinct tx at nonce `n` must come from the bridge's OWN recovery/rotation path — and **same-(sender,nonce) mempool replacement evicts the prior tx** (a payment and its refund are often the *same* event, self-defeating the double). Model it dynamically (three heads: latest/safe/finalized + reorg + mempool eviction) before claiming a double-spend. Related: [[chain-construct]].

## LayerZero / DVN specialization
A single-signer or attacker-compromisable Decentralized Verifier Network (DVN) / oracle-relayer set is a named instance of the **Quorum soundness** + **forge-an-attestation** classes above — not a separate skill.
- **Read the config first (`cast`).** On a LayerZero-style OApp/OFT pathway, read the DVN/verifier set, the required threshold (quorum), and the send/receive library wiring per OApp/pathway. Record N and the threshold. Flag any pathway whose effective quorum is 1 (or trivially small / all controlled by one operator), or where the receive endpoint / `OFTAdapter` trusts a single attestation with no independent second verifier and no fraud window.
- **PoC in a forked two-chain harness.** `anvil --fork` per chain: submit a forged verified message through the configured verifier path and assert the destination releases unbacked assets. Negative control: a healthy multi-DVN quorum rejects the single forged attestation.
- **Dedup / prior art.** KelpDAO, April 2026 — $292M via a 1-of-1 LayerZero DVN compromise + RPC poisoning. The on-chain quorum weakness is the intrinsic, submittable defect; RPC poisoning and backup-node DDoS are the realistic off-chain delivery amplifiers, **not** the finding. Dedup against the KelpDAO / LayerZero-DVN advisories before submission.
