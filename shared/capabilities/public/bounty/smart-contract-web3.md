...
`read-only-reentrancy-check`, `durable-nonce-exploitation`, `cross-chain-bridge-audit`) each map to a
...
gated on all four observable predicates by `systematic-attacking`'s evidence gate at S4 before anything reaches the operator.
