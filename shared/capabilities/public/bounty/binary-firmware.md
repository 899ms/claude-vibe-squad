...
lead. `experimental-attacker` (codex) runs the novel-attack ideation pass as **leads
only** at S2/S3, fanning out to heavy-hitter (Sol / Opus 5) validation and gated on all four observable predicates by
`systematic-attacking`'s evidence gate at S4 — inside operator-provisioned isolation, no laxer bar than known-class
hypotheses (`systematic-attacking` Phase 3b).

**Native exploit-triage probe (2026-08-24).** The selected gadget enumerator is `ropgadget`
: the exact-path v7.7 executable enumerated 43 unique `ret` gadgets from the
trusted host binary `/bin/ls`. It is not on PATH, so the registry records its callable path.
`pwndbg` remains a prose-only `needs_tool` candidate after its standalone command and both GDB
Python probes failed, and the bounded executable census found no candidate. `ropper` was also absent
but is intentionally not added: one live enumerator fills that role without duplicate arsenal noise.
This narrows gadget enumeration only; the card remains `needs_tool`, and none of its other tracked
gaps is closed.
