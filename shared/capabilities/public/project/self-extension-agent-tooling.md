...
| **S3** Produce (build) | `ai-engineer`, `backend-engineer`, `prompt-engineer`, `devops-engineer` | `claude mcp/plugin/agents`, `codex mcp`, `codex mcp-server`, `codex plugin`, `agy mcp`, `agy plugin`, `agy agents`, `codex --sandbox`, `claude --worktree` | — | least-privilege fs/egress/credential/secret handling (hard acceptance) |
| **S4** Verify | `test-engineer`, `software-supply-chain-engineer` | — | `wirework-preflight`; `parity-probe` | typed-failure / no-false-success acceptance: startup · `tools/list` discovery · permissions · negative-path · rollback · tested on **every advertised lane** |
