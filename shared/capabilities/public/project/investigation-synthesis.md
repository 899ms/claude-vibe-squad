...
| **S7** Capture | `Chrono`, `memory-curator` | `chrono-vault` | `claim-verification` | memory overlay (record); sensitive-topic durable-note approval |

**Notes.** `perplexity_search` is Claude-lane only (cite the lane); `xai_search`/`arxiv_search` are
...
is the first-class subscription grounding route at S4, alongside the metered `perplexity_search`. `Brave
