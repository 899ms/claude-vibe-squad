...
audience: specialist
description: Use when a single bug-bounty observation is too weak to pay out but several low-severity leads might compose: turn individually-inert findings into one demonstrated exploit chain whose end state proves realized impact (RCE, auth-bypass, privilege-escalation/ATO, funds theft, private-data). Not for a finding that already reaches impact in one step.
---
...
outcome: chain head, numbered links, objective predicate, reproduced?, preconditions.
